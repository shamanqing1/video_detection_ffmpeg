...
int encode(AVCodecContext *enc_ctx, AVFrame *frame, AVPacket *pkt, std::ofstream &outfile);
...
int main(int argc, char const *argv[])
{
    ...
    std::ofstream outputVideo(argv[2], std::ios::out | std::ios::binary);
    ...
    for(std::string f: files) {
        ...
        pFrame->pts = i++;
        encode(pCodecContext, pFrame, pPacket, outputVideo);
    }


    encode(pCodecContext, nullptr, pPacket, outputVideo);
    outputVideo.close();
    return 0;
}
